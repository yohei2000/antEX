import { FOOD_INCOME_MULTIPLIER } from "../config/balance";
...
export function computeDerivedColony(colony: ColonyState, options: ComputeDerivedOptions = {}): DerivedColonyState {
  ...
  const heavyTarget = heavySoldierBrood > 0 ? soldierAnts : 0;
  const heavySoldiers = Math.floor(clamp(colony.heavySoldierAnts, 0, heavyTarget));
  const shieldHeadTarget = shieldHeadBrood > 0 ? Math.max(0, soldierAnts - heavySoldiers) : 0;
  const shieldHeads = Math.floor(clamp(colony.shieldHeadAnts, 0, shieldHeadTarget));
  const acidShooterTarget = acidShooterBrood > 0 ? Math.max(0, soldierAnts - heavySoldiers - shieldHeads) : 0;
  const acidShooters = Math.floor(clamp(colony.acidShooterAnts, 0, acidShooterTarget));
  const scoutTarget = scoutBrood > 0 ? Math.max(0, soldierAnts - heavySoldiers - shieldHeads - acidShooters) : 0;
  const scouts = Math.floor(clamp(colony.scoutAnts, 0, scoutTarget));
  const captainTarget = captainBrood > 0 ? Math.max(0, soldierAnts - heavySoldiers - shieldHeads - acidShooters - scouts) : 0;
  ...
  const builderTarget = builderTraining > 0 ? availableWorkers : 0;
