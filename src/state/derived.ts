...
export function computeDerivedColony(colony: ColonyState, options: ComputeDerivedOptions = {}): DerivedColonyState {
  ...
  const specialSoldierCapacity = (level: number) => level > 0 ? level * 2 : 0;
  const heavyTarget = Math.min(soldierAnts, specialSoldierCapacity(heavySoldierBrood));
  const heavySoldiers = Math.floor(clamp(colony.heavySoldierAnts, 0, heavyTarget));
  const shieldHeadTarget = Math.min(Math.max(0, soldierAnts - heavySoldiers), specialSoldierCapacity(shieldHeadBrood));
  const shieldHeads = Math.floor(clamp(colony.shieldHeadAnts, 0, shieldHeadTarget));
  const acidShooterTarget = Math.min(Math.max(0, soldierAnts - heavySoldiers - shieldHeads), specialSoldierCapacity(acidShooterBrood));
  const acidShooters = Math.floor(clamp(colony.acidShooterAnts, 0, acidShooterTarget));
  const scoutTarget = Math.min(Math.max(0, soldierAnts - heavySoldiers - shieldHeads - acidShooters), specialSoldierCapacity(scoutBrood));
  const scouts = Math.floor(clamp(colony.scoutAnts, 0, scoutTarget));
  const captainTarget = Math.min(Math.max(0, soldierAnts - heavySoldiers - shieldHeads - acidShooters - scouts), specialSoldierCapacity(captainBrood));
  ...
  const normalSoldiers = Math.max(0, soldierAnts - heavySoldiers - shieldHeads - acidShooters - scouts - captains);
  ...
  const attackPower = 1 + soldierTraining * 0.15 + sentinelPosts * 0.03 + normalSoldiers * 0.002 + heavySoldiers * 0.01 + shieldHeads * 0.004 + acidShooters * 0.006 + scouts * 0.003 + captains * 0.005;
  const defensePower =
