...
export function computeDerivedColony(colony: ColonyState, options: ComputeDerivedOptions = {}): DerivedColonyState {
  ...
  const medicTarget = medicBrood > 0 ? Math.max(0, soldierAnts - heavySoldiers - shieldHeads - acidShooters - scouts) : 0;
  const medics = Math.floor(clamp(colony.medicAnts, 0, medicTarget));
  const captainTarget = captainBrood > 0 ? Math.max(0, soldierAnts - heavySoldiers - shieldHeads - acidShooters - scouts - medics) : 0;
