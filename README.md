...
最初は小さな巣と 12 匹程度のアリから始まります。時間経過で食料が増え、育成キューでアリを増やし、巣の収容上限、兵隊数、土木アリを伸ばしていきます。プレイヤーは強化、育成、土木工事、兵隊出撃を選びながら、採餌、敵襲、防衛、負傷を管理します。
...
- 初期 12 匹の小規模コロニー
- 時間経過による食料と敵脅威の増加、育成キューによるアリ数増加
...
- 軍事タブからの一時出撃と敵襲迎撃
- 重兵装アリと土木アリの巣内リソース管理
- 土木タブからの採餌道整備、低い土塁、土壁、見張り塚の発注
- ボクセル風ブロックによる土木建築物の進捗描画
- 重兵装、盾頭、酸射、斥候、救護、小隊長、土木アリによる役割分化
- GitHub Pages 配信

## Game Loop

1. 働きアリが食料を集める
2. 育房タブで働きアリ、土木アリ、兵隊アリを育成キューへ入れる
3. 強化で採餌効率、育成速度、収容上限、戦闘力を上げる
4. 土木アリを工事に割り当て、採餌道や低い土塁を必要な場所へ発注する
5. 種別ごとの食料コストと育成時間を読み、キュー完了で1匹ずつ増やす
6. 敵襲時に軍事タブから出撃させる
7. 敵襲の接近、開始、撃退または被害を確認する
8. 外敵や敵脅威に備えながら、より大きな巣を目指す
...
- ピンチ: ズーム
- 成長タブ: 巣と資源の状態確認、強化購入
- 土木タブ: 土木アリの状態確認、採餌道整備、防衛建築、見張り塚の発注、作業ごとの担当数調整
- 育房タブ: 働きアリ、土木アリ、兵種ごとの食料コストと育成時間を見て、育成キューへ追加
- 軍事タブ: 巣内兵隊の確認、兵隊出撃

通常アプリから遠征モードは削除済みです。働きアリ、土木アリ、兵隊アリ、特殊兵はすべて育房タブの育成キューで1匹ずつ増えます。アリ種ごとの育成上限はなく、巣全体の収容上限と解禁条件だけが制限になります。兵隊アリは平時は巣内リソースとして扱い、プレイヤーが軍事タブから指示した時だけ地表へ出撃します。

土木アリも平時は巣内リソースとして扱い、土木タブから工事に割り当てられた時だけ地表へ出ます。各工事は開始時1匹担当で、作業ごとに1〜10匹の範囲で増減できます。土塁、土壁、見張り塚はボタン押下後に地面をクリックして設置場所を指定します。土木工事は完全な地形変形ではなく、軽量な範囲効果と視覚表現として扱います。
...
- `src/config/variants.ts`: `worker`, `soldier`, `heavySoldier`, `shieldHead`, `acidShooter`, `scout`, `medic`, `captain`, `builder` などのアリ種別定義
