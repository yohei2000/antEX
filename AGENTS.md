...
- 完成済み `sentryMound` は配置地点から広い現在視界を作る。完成済み `earthWall` と `lowBarricade` も周辺に小さな現在視界を作る。未完成の土木工事や `trailReinforce` は視界源にしない。
...
- アリが地表で入った領域はランタイム中の探索済み領域として記録し、未踏破の暗闇より薄いグレーの記憶フォグで地形だけを見せる。敵、餌場の現在状態、通常の敵巣表示は巣周辺または地表アリの現在視界に入った時だけ見せる。斥候インテルで発見した敵巣位置は遠征導線として例外的に扱える。現時点ではセーブ対象にしない。
...
- 敵巣耐久を0にして敵巣を陥落させると勝利としてゲーム終了する。
- プレイヤー巣は巣耐久を持つ。敵襲中に敵が巣周辺へ侵入して breach が発生した時だけ巣耐久を削り、餌場被害だけでは巣耐久を削らない。
- 貯蔵食料が敵襲で減るのは、敵が巣周辺へ侵入して本拠地 breach が発生した時だけにする。餌場を荒らされただけでは即時に食料ストックを減らさない。
- 巣耐久が0になると女王が倒され、敗北としてゲーム終了する。ゲーム終了後は新しい巣で再開するまで成長、敵襲、出撃、育成、土木を進めない。
