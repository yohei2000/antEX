...
- 一度の出撃指示で出せる兵隊アリは、通常兵隊アリ、重兵装アリ、盾頭アリ、酸射アリを合算した総数の半分までにする。端数は切り上げ、総数1匹の時は1匹出撃可能にする。これは1回あたりの出撃上限であり、クールダウン後は巣内に残っている兵隊を追加出撃できる。
- 重兵装アリも通常兵隊アリと同じく巣内兵力として扱い、平時は地表に出さない。プレイヤーが兵隊タブから出撃指示した時だけ地表に表示する。
- 盾頭アリも巣内兵力として扱い、平時は地表に出さない。出撃時は遠くへ追撃せず、巣口や狭い通路を封鎖する。敵とは組み合い戦闘を発生させず、味方を守る壁として敵を押し返す。
